...
int receive_msg(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    int rc;
    fd_set rfds;
    struct timeval tv;
...
int receive_msg_OLD(modbus_t *ctx, uint8_t *msg, msg_type_t msg_type)
{
    int rc;
    fd_set rfds;
    struct timeval tv;
...
/* Receives the response and checks values.
   ...
   these values. */
int receive_msg_req(modbus_t *ctx, uint8_t *req, uint8_t *rsp)
{
    int rc;
    ...
    } else if (rc == (int)(offset + 2 + ctx->backend->checksum_length) &&
...
int modbus_write_bits(modbus_t *ctx, int addr, int nb, const uint8_t *src)
{
    ...
    unsigned long starTime=getTick();
    unsigned long Spedizione,Ricezione;
    if (rc > 0) {
        Spedizione= getTick()-starTime;
        starTime=getTick();
        uint8_t rsp[MAX_MESSAGE_LENGTH];
        rc = receive_msg_req(ctx, req, rsp);
        Ricezione= getTick()-starTime;
        //fprintf(stderr, "ERROR Writing too many bits (%d > %d)\n",
        //        nb, MODBUS_MAX_WRITE_BITS);
    }
...
void modbus_poll(modbus_t* ctx)
{
    ...
    ctx->backend->recv(ctx, msg, msg_len);
